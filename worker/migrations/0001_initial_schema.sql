-- TREK base schema (ported from server/src/db/schema.ts)
-- D1 is SQLite-compatible, so the schema transfers nearly verbatim.
-- Differences vs. better-sqlite3:
--   * DATETIME DEFAULT CURRENT_TIMESTAMP works identically.
--   * No PRAGMA foreign_keys=ON needed; D1 enforces them by default.
--   * AUTOINCREMENT works identically.
...
-- trip_files references reservations(id), so it must be declared after reservations.
